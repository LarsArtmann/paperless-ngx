...
const REQUESTS_HAR2 = path.join(
	__dirname,
	"requests/api-document-detail2.har",
);
...
test("should activate / deactivate save button when changes are saved", async ({
	...
	await page.waitForSelector(
		"pngx-document-detail pngx-input-text:first-child",
	);
...
test("should support quick filters", async ({ page }) => {
	...
	await expect(page).toHaveURL(
		/tags__id__all=4&sort=created&reverse=1&page=1/,
	);
});
