...
describe("DocumentDetailComponent", () => {
	...
	it("should not render a delete button for the root/original version", () => {
		...
		jest
			.spyOn(documentService, "get")
			.mockReturnValueOnce(of(docWithVersions));
		jest
	...
	it("should not attempt to retrieve objects if user does not have permissions", () => {
		...
		httpTestingController.expectNone(
			`${environment.apiBaseUrl}documents/tags/`,
		);
		httpTestingController.expectNone(
	...
	it("should not close if e.g. user-cancelled", () => {
		initNormally();
		jest
			.spyOn(openDocumentsService, "closeDocument")
	...
	it("should detect RTL languages and add css class to content textarea", () => {
		...
		expect(
			fixture.debugElement.queryAll(By.css("textarea.rtl")),
		).not.toBeNull();
	});
	...
	it("should display built-in pdf viewer if not disabled", () => {
		...
		fixture.detectChanges();
		expect(
			fixture.debugElement.query(By.css("pngx-pdf-viewer")),
		).not.toBeNull();
	});
	...
	it("should show incoming update modal when open local draft is older than backend on init", () => {
		...
		const remoteDoc = Object.assign({}, doc, {
			modified: new Date(
				new Date(doc.modified).getTime() + 1000,
			).toISOString(),
		});
		jest
			.spyOn(activatedRoute, "paramMap", "get")
		...
		const confirmDialog =
			openModal.componentInstance as ConfirmDialogComponent;
	...
	it("selectVersion should update preview and handle preview failures", () => {
		...
		jest
			.spyOn(documentService, "getThumbUrl")
			.mockReturnValue("thumb-version");
		jest
			.spyOn(documentService, "get")
		...
		expect(component.documentForm.get("content").value).toBe(
			"version-content",
		);
		const pdfSource = component.pdfSource as {
			url: string;
			password?: string;
		};
	...
	iframePrintErrorCases.forEach(
		({ description, thrownError, expectToast }) => {
			it(
				description,
				fakeAsync(() => {
					initNormally();
