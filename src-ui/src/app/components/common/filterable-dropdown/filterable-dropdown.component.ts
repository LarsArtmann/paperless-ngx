...
export class FilterableDropdownSelectionModel {
	...
	getSelectedItems() {
		return this.items.filter(
			(i) =>
				this.temporarySelectionStates.get(i.id) ==
	...
	toggleIntersection() {
		...
		this.temporarySelectionStates.forEach((state, key) => {
			if (key === null && this.intersection === Intersection.Exclude) {
				this.temporarySelectionStates.set(
					NEGATIVE_NULL_FILTER_VALUE,
					newState,
				);
			} else if (
	...
	clear(fireEvent = true) {
		...
		this.temporaryLogicalOperator = this._logicalOperator =
			LogicalOperator.And;
	...
	isDirty() {
		if (
			!Array.from(this.temporarySelectionStates.keys()).every(
				(id) =>
					this.temporarySelectionStates.get(id) ==
					this.selectionStates.get(id),
			)
		) {
			return true;
		} else if (
