...
const FILTER_HANDLERS: Record<TriggerFilterType, FilterHandler> = {
	[TriggerFilterType.TagsAny]: {
		apply: (aggregate, values) => {
			aggregate.filter_has_tags = Array.isArray(values)
				? [...values]
				: [values];
		},
	...
	implements OnInit
{
	...
	removeFilter(triggerFormGroup: FormGroup, filterIndex: number) {
		...
		if (
			filterGroup?.get("type").value === TriggerFilterType.CustomFieldQuery
		) {
		...
		this.actionFields.push(
			new FormGroup({
				...
				webhook: new FormGroup({
					...
					include_document: new FormControl(
						!!action.webhook?.include_document,
					),
				}),
