...
describe("WorkflowEditDialogComponent", () => {
	...
	it("should update order on drag n drop", () => {
		...
		component.object = workflow;
		component.ngOnInit();
		component.onActionDrop({
			previousIndex: 0,
			currentIndex: 1,
		} as CdkDragDrop<WorkflowAction[]>);
	...
	it("should prune empty nested objects on save", () => {
		...
		expect(
			component.objectForm.get("actions").value[0].webhook,
		).not.toBeNull();
		component.save();
	...
	it("should map filter builder values into trigger filters on save", () => {
		...
		const correspondentAny = addFilterOfType(
		...
		const correspondentNot = addFilterOfType(
	...
	it("should ignore empty and null filter values when mapping filters", () => {
		...
		correspondentFilter
			.get("type")
			.setValue(TriggerFilterType.CorrespondentIs);
	...
	it("should expose select metadata helpers", () => {
		expect(
			component.isSelectMultiple(TriggerFilterType.CorrespondentAny),
		).toBe(true);
		expect(
			component.isSelectMultiple(TriggerFilterType.CorrespondentNot),
		).toBe(true);
	...
	it("should recover from invalid custom field query json and update control on changes", () => {
		...
		expect(valuesControl.value).toEqual(
			JSON.stringify(expression.serialize()),
		);
