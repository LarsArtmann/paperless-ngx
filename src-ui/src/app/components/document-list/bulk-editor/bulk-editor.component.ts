	...
	implements OnInit, OnDestroy
{
	...
	get userCanEditAll(): boolean {
		...
		const docs = this.list.documents.filter((d) =>
			...
			.pipe(
				switchMap((newDocumentType) => {
					return this.documentTypeService
						.listAll()
						.pipe(
							map((documentTypes) => ({ newDocumentType, documentTypes })),
						);
				}),
			)
			.pipe(takeUntil(this.unsubscribeNotifier))
	...
	rotateSelected() {
		...
		const rotateDialog =
			modal.componentInstance as RotateConfirmDialogComponent;
