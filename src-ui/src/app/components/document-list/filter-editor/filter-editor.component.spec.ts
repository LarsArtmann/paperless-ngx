...
describe("FilterEditorComponent", () => {
	...
	it("should ingest filter rules for has storage path", fakeAsync(() => {
		expect(
			component.storagePathSelectionModel.getSelectedItems(),
		).toHaveLength(0);
		component.filterRules = [
			{
	...
	it("should ingest filter rules for does not have any of storage paths", fakeAsync(() => {
		expect(
			component.storagePathSelectionModel.getExcludedItems(),
		).toHaveLength(0);
		component.filterRules = [
			{
	...
	it("should ingest filter rules for custom fields all", fakeAsync(() => {
		...
		expect(component.customFieldQueriesModel.queries[0].value.length).toEqual(
			2,
		);
		expect(
			(
	...
	it("should convert user input to correct filter rules on correspondent selections", fakeAsync(() => {
		...
		correspondentsFilterableDropdown.componentInstance.dropdownOpenChange(
			true,
		);
		fixture.detectChanges();
		...
		const toggleIntersectionButtons =
			correspondentsFilterableDropdown.queryAll(By.css("input[type=radio]"));
	...
	it("should convert user input to correct filter on permissions select shared with me", fakeAsync(() => {
		...
		const sharedWithMeButton = permissionsDropdown.queryAll(
			By.css("button"),
		)[3];
	...
	it("should emit reset filter rules when resetting", () => {
		...
		const filterRulesChangeSpy = jest.spyOn(
			component.filterRulesChange,
			"next",
		);
	...
	it("should support Enter / Esc key on text field", () => {
		...
		const blurSpy = jest.spyOn(
			component.textFilterInput.nativeElement,
			"blur",
		);
		component.textFilterInput.nativeElement.dispatchEvent(
			new KeyboardEvent("keyup", { key: "Escape" }),
