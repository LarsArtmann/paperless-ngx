	...
	getProgress(): number {
		switch (this.phase) {
			...
			case FileStatusPhase.UPLOADING:
				return (
					(this.currentPhaseProgress / this.currentPhaseMaxProgress) * 0.2
				);
			case FileStatusPhase.WORKING:
				return (
					(this.currentPhaseProgress / this.currentPhaseMaxProgress) * 0.8 +
					0.2
				);
...
export class WebsocketStatusService {
	...
	private readonly documentConsumptionFailedSubject =
		new Subject<FileStatus>();
	...
	getConsumerStatusCompleted() {
		return this.consumerStatus.filter(
			(s) =>
				s.phase == FileStatusPhase.FAILED ||
				s.phase == FileStatusPhase.SUCCESS,
		);
	}
