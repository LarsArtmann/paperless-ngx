...
export class DocumentListViewService {
		...
		this.documentService
			...
			.subscribe({
				next: (result) => {
					...
					this.rangeSelectionAnchorIndex = this.lastRangeSelectionToIndex =
						null;
				},
				error: (error) => {
					this.isReloading = false;
	...
	get displayMode(): DisplayMode {
		const mode =
			this.activeListViewState.displayMode ?? DisplayMode.SMALL_CARDS;
	...
	get displayFields(): DisplayField[] {
		return (
			this.activeListViewState.displayFields ?? LIST_DEFAULT_DISPLAY_FIELDS
		);
	}
